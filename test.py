...
class ConfigEditorDialog(QDialog):
    ...
    def __init__(self, yaml_path, parent=None):
        ...
        self.right_form.setFieldGrowthPolicy(QFormLayout.ExpandingFieldsGrow)
    ...
    def build_form(self):
        ...
        def add_list(key, items_list=None, help_text=None, column=None):
            ...
            lst_widget.setSelectionMode(QListWidget.MultiSelection)
        ...
        add_dropdown("solver", ["cplex", "gurobi", "glpk", "cbc", "xpress", "knitro"],
            help_text="Select the solver to use for optimization. Recommended 'cplex' or 'gurobi'", column="left")
        ...
        add_int("DA_lookahead_periods", 12, 6, 24,
            help_text="How many hours to look ahead in day-ahead SCUC (6-24 hours)", column="left")
        add_int("RT_resolution", 60, 5, 60, 5,
            help_text="Resolution of real-time SCED (5-60 minutes in 5 minute intervals).", column="left")
        add_int("RT_lookahead_periods", 1, 1, 100,
            help_text="How many periods to look ahead in real-time SCED", column="left")
        add_float("mipgap", 0.01, help_text="MIP gap for the solver", column="left")
        add_dropdown("run_RTSCED_as", ["LP", "MILP"],
            help_text="LP is faster but MILP gives better commitment decisions", column="left")
        add_dropdown("load_timeseries_aggregation_level", ["node", "area"],
            help_text="How your load_timeseries data is arranged columnwise", column="left")
        add_int("storage_AS_participation_level", 4, 0, 4,
            help_text="How many ancillary services can ESS participate in one time-period", column="left")
        add_checkbox("branch_contingency",
            help_text="Enable N-1 transmission security constraints", column="left")

        add_list("thermal_generator_types", items_list=["CT", "CC", "STEAM", "NUCLEAR"],
            help_text="Select the thermal unit types in gen.csv file", column="right")
        add_list("renewable_generator_types", items_list=["PV", "RTPV", "CSP", "HYDRO", "WIND"],
            help_text="Select all renewable unit types in gen.csv file", column="right")
        add_list("fixed_renewable_types", items_list=["RTPV"],
        ...
        for key in reserve_keys:
            add_dropdown(key, reserve_options,
        ...
        add_checkbox("plotly_plots",
            help_text="Generate html plots for better illustration (takes longer time)", column="right")
        add_dropdown("output_interval", ["at_once", "daily", "weekly", "monthly"],
            help_text="How often to generate plots and json output", column="right")
